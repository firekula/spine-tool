...
工具会还原 Atlas 的旋转、裁剪、偏移和 `orig` 逻辑尺寸，并从页级 `scale` 或 Region 附件尺寸推算导出倍率。Atlas 的 `rotate: true` 表示打包时把源图逆时针旋转 90°，数字 `90`/`270` 同样表示打包角度；恢复时应用其逆变换。页级 `size` 可以省略或写为官方未知尺寸形式 `0,0`，工具会使用解码后的 PNG 实际尺寸。若 Spine 以 50% 导出且 Atlas 记录 `scale: 0.5`，工具会按 2 倍恢复逻辑尺寸。界面会显示高、中、低置信度；低置信度代表证据不足或存在异常尺寸，请在导出前核对，必要时修改全局倍率或单个 Region 倍率。

部分素材的 PNG 右侧和底部比 Atlas 声明的页面 `size` 少若干像素（常见于导出后被裁掉整行/整列透明像素的图片）。工具按官方拆图行为处理：页面框取 Atlas 声明尺寸与 PNG 解码尺寸的并集，PNG 之外的像素视为完全透明，因此这些 Region 仍能导出完整尺寸的图片，而不是整份 Atlas 受控失败。长度不一致时会输出“PNG 小于 Atlas 声明尺寸”诊断，列出实际尺寸、声明尺寸、补齐方向和受影响的 Region；报告的 `sourcePadding` 字段记录单个 Region 用到的补齐量。只有同时超出声明尺寸和 PNG 实际尺寸的 Region 才会在恢复前以 `REGION_OUT_OF_BOUNDS` 受控拒绝。
