...
`scripts/runtime-sources.json` 固定 3.5–4.3 八条官方来源。当前已集成的 3.8–4.2 Runtime 在构建时进入各自的动态 chunk；3.5、3.6、3.7 与 4.3 只固定来源或依赖，等对应 adapter 与 vendor 资产真实存在后才纳入 chunk 验证。任何 `runtime-X_Y.ts` 只导入其对应的官方包或 vendor 构建；4.x 的 npm alias 各自解析到同版本 `@esotericsoftware/spine-core`，不能跨版本复用 core。
...
- 3.5 与 3.6 固定 commit 中 `spine-ts/LICENSE` 的 SHA-256 为 `d2af98ecac7e4bb6e4c4491fc734db7762b94626b18bcc87c7eac6febd86e1b5`；3.7 的为 `6142ee6cc2c03d3a918793e4750ae772bd3755c534d4a35e559e301acf51ec39`。在各 vendor 集成前只记录官方来源 hash，不声明本地副本已验证。
